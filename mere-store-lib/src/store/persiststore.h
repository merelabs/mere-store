...
class Pair;
class PersistStore : public BaseStore
{
    Q_OBJECT
public:
    virtual ~PersistStore();
    explicit PersistStore(const QString &store, QObject *parent = nullptr);
    explicit PersistStore(const QString &store, const QString &slice, QObject *parent = nullptr);
...
private:
    class PersistStorePrivate;
    PersistStorePrivate *d_ptr;
};
